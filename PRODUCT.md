...
- Two distinct Lucy widgets exist, confirmed by the user 2026-08-14: the demo widget (ID `6a79aa9de427c78bb5bcaca5`, "ABC Plumbing, Watford" persona) used on the homepage's Test Lucy section for stress-testing, and a separate real-enquiries widget (ID `6a7f08c932ce808db2dd579f`, "NOLY AI Website" branding, a particle-orb voice UI rather than the demo's phone-card UI) used on /contact for actual bookings. CallLucyCard now takes a `widgetId` prop so both reuse the same reparenting logic; do not conflate the two IDs or point real enquiries at the demo persona.
- All site-wide "Get Started" CTAs (Nav, Pricing, Test Lucy section, /story) now point to /contact, where the real-enquiries Lucy widget is the primary path in, with phone/email kept as a manual fallback underneath.
